...
Measured on the included synthetic benchmark (`npm run bench`), Node v22.22.2, single core, full pipeline (extract → scrub → dedupe → coverage → JSONL). Throughput is hardware-bound and will move with your CPU — these numbers are from a shared Xeon @ 2.10GHz container, re-measured 2026-08-24 because the previous figures (~183k/~189k spans/s) didn't reproduce here; run `npm run bench` on your own machine rather than trusting either table:

| spans | cases out | time | throughput |
|---|---|---|---|
| 11,000 | 8,001 | 150 ms | ~73,000 spans/s |
| 55,000 | 40,001 | 1,064 ms | ~52,000 spans/s |

A day of traces is still a coffee-sip, not a batch job — and the throughput number isn't really the comparison that matters. The alternative to `trace2eval` isn't a faster script, it's a human: reading each span, writing an expected output by hand, deduping by eye. That's hours-to-days of work for a few thousand cases, not milliseconds. `trace2eval` doesn't out-run manual curation, it replaces it — which is what lets an eval set track production traffic instead of drifting stale between hand-written batches.
